...
void eval(char *cmdline) 
{
    ...
    char buf[MAXLINE];   // 保存cmdline
    int bg;              // parseline返回值，代表是bg还是fg job
    pid_t pid;           // 进程pid
    sigset_t mask_one, mask_all, oldmask; //阻塞信号集
    ...
    if(!builtin_cmd(argv)){ // 不是内置命令
        ...
        */
        ...
        if (!bg){ // 父进程循环等待foreground job终止
            ...
            global_pid = 0;
            while( !global_pid )
                /* sigsuspend暂时使用oldmask代替当前阻塞集，并挂起进程节省资源;
                收到当前非阻塞集信号后唤醒进程继续执行 */
                Sigsuspend(&oldmask);              
            Sigprocmask(SIG_SETMASK, &oldmask, NULL);
        }

        else{ // background job 则打印信息
...
int builtin_cmd(char **argv) 
{
    ...
    if (!strcmp(argv[0], "&"))    // 忽略&
        return 1;
    if (!strcmp(argv[0], "jobs")){ // jobs命令，列出所有background job
        int i;
        for (i = 0; i < MAXJOBS; i++) {
            if (jobs[i].pid != 0 ) {
                char * status_ = "";
                if (jobs[i].state == 2)
                    ...
                    status_ = "Stopped";
                printf("[%d] (%d) %s %s", jobs[i].jid, jobs[i].pid, status_, jobs[i].cmdline);
            }
        }
        return 1;
    } 
    if(!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")){
        if (argv[1] == NULL || argv[2] != NULL){    // 判断输入参数个数是否为2
...
void do_bgfg(char **argv) 
{
    ...
    if (*(char *)(argv[1]) != '%'){
        ...
        struct job_t *temp_j = getjobjid(jobs, jid);
        if(temp_j == NULL){
    ...
    if (!strcmp(argv[0], "bg")){ // bg命令
        ...
        sigset_t mask_one, oldmask; //阻塞信号集
        Sigemptyset(&mask_one);
        Sigaddset(&mask_one, SIGCHLD);
        Sigprocmask(SIG_BLOCK, &mask_one, &oldmask); // 先阻塞SIGCHLD信号
        global_pid = 0;
        while( !global_pid )
            /* sigsuspend暂时使用oldmask代替当前阻塞集，并挂起进程节省资源 */
            Sigsuspend(&oldmask);              
